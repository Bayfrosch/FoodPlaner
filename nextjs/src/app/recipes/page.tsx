...
export default function RecipesPage({ shoppingLists }: RecipesPageProps) {
    ...
    return (
      <div className="flex items-center justify-center py-8 sm:py-12">
        <div className="text-center">
          ...
          <p className="text-xs sm:text-base text-gray-400">Lädt Rezepte...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 text-red-400 px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-2xl mb-4 sm:mb-6 flex items-center justify-between">
      ...
      {/* Recipes Grid */}
      {allRecipes.length === 0 ? (
        <div className="bg-gradient-to-br from-[#14141f]/50 to-[#1a1a2e]/50 border border-purple-500/20 rounded-2xl sm:rounded-3xl p-8 sm:p-12 text-center">
          <svg className="w-12 sm:w-16 h-12 sm:h-16 text-purple-500/30 mx-auto mb-3 sm:mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6m0 0v6m0-6h6m0 0h6m-6-6h6m0 0h6M6 12h6m0 0h6" />
          </svg>
          <h3 className="text-base sm:text-lg font-semibold text-gray-300 mb-2">Noch keine Rezepte</h3>
          <p className="text-xs sm:text-base text-gray-400 mb-4 sm:mb-6">Erstellen Sie Ihr erstes Rezept, um es später zu Ihren Einkaufslisten hinzuzufügen</p>
          <button
            onClick={() => handleOpenModal()}
            className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white rounded-lg sm:rounded-2xl font-bold transition-all text-xs sm:text-base"
          >
            Rezept erstellen
          </button>
        </div>
